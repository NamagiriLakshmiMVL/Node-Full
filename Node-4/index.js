...
app.get("/products", (req, res) => {
    ...
    // let newArr = products
    // if (category) {
    //     newArr = newArr.filter((product) => product.category === category)
    // }
    // if (rating) {
    //     newArr = newArr.filter((product) => product.rating === +rating)
    // }
    let newArr = products.filter((val)=>(
        (!category || val.category === category) && (!rating || val.rating===+rating)
    ))
    res.send(newArr)
})

app.listen(PORT, () => console.log("Server Statred", PORT))